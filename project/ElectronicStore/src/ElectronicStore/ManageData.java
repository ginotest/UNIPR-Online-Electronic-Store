...
public abstract class ManageData {
	...
	public static void addData(String type, String[] content)  {
		...
		File fXmlFile = new File(".\\src\\ElectronicStore\\" + xmlFile);
	...
	public static void removeData(String type, String elementToDelete) {
		...
		File file = new File(".\\src\\ElectronicStore\\" + xmlFile);
	...
	public static ArrayList<String> getProfile(String type, String usrn){
		...
		try {
			...
			dom = db.parse(".\\src\\ElectronicStore\\"+xmlFile);
	...
	public static Element docBuilder(String file) {
		...
		try {
			DocumentBuilder db = dbf.newDocumentBuilder();
			dom = db.parse(".\\src\\ElectronicStore\\"+file);
		} catch (Exception e)    {
			e.printStackTrace();  
		} 
	...
	public static void save(Document doc, String output) {
		...
		try {
			Transformer transformer = TransformerFactory.newInstance().newTransformer(new StreamSource(new File(".\\src\\ElectronicStore\\trim-whitespace.xslt")));
			transformer.setOutputProperty(OutputKeys.INDENT, "yes");
			transformer.setOutputProperty(OutputKeys.STANDALONE, "no");
			transformer.transform(new DOMSource(doc), new StreamResult(".\\src\\ElectronicStore\\" + output));
		} catch (Exception e)    {
			e.printStackTrace();  
		} 
	}
	...
	public static void editQuantity(String user){
		...
		int fixedIndex = 0, indexProductChosen, newQuantity;
